...
3. **提交與推播 (Commit & Push)**:
   ...
   * **Commit 訊息格式規範 (Commit Message Generation Rules)**：
     當生成 Git Commit Message 時，**必須**嚴格遵守以下結構化格式：
     1. **Header**：使用 Conventional Commits 格式（例如：`feat(scope): short description`）。
     2. **Body**：**必須**以項目清單（`- Item`）方式列出所有具體的工作項目。每個項目必須準確反映程式碼中的邏輯變更（例如：`- Added tool X`, `- Refactored function Y`, `- Registered config Z`）。
